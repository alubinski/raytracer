...
int main() {
  ...
  const point_t lightPos = Point(0.f, 19.f, 0.f);
  world.addLight(PointLight(lightPos, {1.f, 1.f, 1.f}));
...

  {
    ...
    p->setTransformation(scaling(.2f, .2f, .2f));
    m.setPattern(p);
...

  {
    ...
    lamp->setTransformation(translation(lightPos.x, lightPos.y, lightPos.z) *
                            scaling(2.f, .5f, 2.f));
    lamp->setCastsShadows(false);
    auto &m = lamp->material();
